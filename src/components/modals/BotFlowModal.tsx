...
import { BotFlow, FlowStep } from '../../types';
...
const BotFlowModal: React.FC<BotFlowModalProps> = ({ flow, onClose }) => {
  const { addBotFlow, updateBotFlow } = useApp();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (flow) {
      updateBotFlow(flow.id, flowData);
    } else {
      addBotFlow(flowData);
    }
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h3 className="text-lg font-semibold text-gray-900">
            {flow ? 'Editar Fluxo' : 'Novo Fluxo do Bot'}
          </h3>
          <button
            onClick={onClose}
              ...
              {formData.triggers.map((trigger, index) => (
                <div key={index} className="flex gap-2">
                  <input
                    ...
                    placeholder="Ex: olá, oi, produto, preço"
                  />
